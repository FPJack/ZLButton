...
- (void)viewDidLoad
{
    ...
    // 1. 水平 - 图片在左，文字在右（默认）
    {
        ZLButton *btn = [self makeButtonWithTitle:@"确认" color:[UIColor systemBlueColor]];
...
//            [btn setTitle:@"addButtonaddButtonaddButton" forState:UIControlStateNormal];
    ...
    // 1.5 图片偏移 + 文字偏移
    {
        ZLButton *btn = [self makeButtonWithTitle:@"图片下移+文字右移" color:[UIColor systemTealColor]];
        btn.axis = ZLButtonAxisHorizontal;
        btn.layoutSpacing = 8;
    ...
    // 1.6 垂直排列 + 偏移（链式写法）
    {
        ZLButton *btn = [self makeButtonWithTitle:@"偏移示例" color:[UIColor systemPinkColor]];
    ...
    // 2. 水平 - 文字在左，图片在右
    {
        ZLButton *btn = [self makeButtonWithTitle:@"下一步" color:[UIColor systemOrangeColor]];
        btn.axis = ZLButtonAxisHorizontal;
        btn.layoutOrder = ZLButtonOrderTitleFirst;
        btn.layoutSpacing = 8;
    ...
    // 3. 垂直 - 图片在上，文字在下
    {
        ZLButton *btn = [self makeButtonWithTitle:@"相册" color:[UIColor systemGreenColor]];
        btn.axis = ZLButtonAxisVertical;
        btn.layoutOrder = ZLButtonOrderImageFirst;
        btn.layoutSpacing = 6;
    ...
    // 4. 垂直 - 文字在上，图片在下
    {
        ZLButton *btn = [self makeButtonWithTitle:@"下载" color:[UIColor systemPurpleColor]];
        btn.axis = ZLButtonAxisVertical;
        btn.layoutOrder = ZLButtonOrderTitleFirst;
        btn.layoutSpacing = 6;
    ...
    // 5. 固定图片大小 + 圆角 + 内边距
    {
        ZLButton *btn = [self makeButtonWithTitle:@"收藏" color:[UIColor systemRedColor]];
    ...
    // 6. 大内边距 + 大间距
    {
        ZLButton *btn = [self makeButtonWithTitle:@"立即购买" color:[UIColor systemIndigoColor]];
    ...
    // 7. 弹性间距 - 水平
    {
        ZLButton *btn = [self makeButtonWithTitle:@"设置" color:[UIColor systemBlueColor]];
    ...
    NSArray *alignValues = @[@(ZLButtonContentAlignmentCenter),
                             @(ZLButtonContentAlignmentStart),
                             @(ZLButtonContentAlignmentEnd)];
    ...
    for (NSInteger i = 0; i < 3; i++) {
        ...
        btn.axis = ZLButtonAxisHorizontal;
        btn.layoutSpacing = 8;
        btn.layoutContentAlignment = (ZLButtonContentAlignment)[alignValues[i] unsignedIntegerValue];
...
- (void)addButton:(ZLButton *)btn label:(NSString *)text {
    btn.translatesAutoresizingMaskIntoConstraints = NO;
...
- (void)addFullWidthButton:(ZLButton *)btn label:(NSString *)text height:(CGFloat)h {
    btn.translatesAutoresizingMaskIntoConstraints = NO;
...
- (ZLButton *)makeButtonWithTitle:(NSString *)title color:(UIColor *)color {
    ZLButton *btn = [[ZLButton alloc] init];
